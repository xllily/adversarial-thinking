...
Adversarial Thinking 是我为 Coding Agent 做的一个实验性 Agent Skill。它用于重要或难以回退的决策点，此时遗漏一个前提就可能实质改变决定。

我做它，是因为 Agent 很容易沿着一条看起来合理的路线走得太远。这个 Skill 会要求它提出另一种可信解释，再找出可能改变下一步行动、成本最低的检查。当前模型和反模型接受同一套证据标准；保留原方案也是有效结果。检查结束后，控制权回到原工作流。

当一项重要主张需要审慎评审、高成本决策的搜索空间收窄得太早，或连续同类失败已经和现有证据对不上时，用它。日常、可逆且测试充分的工作默认跳过，除非你明确要求检查。

`0.1.1` 仍是实验版本。现有评估不能证明它比默认模型更好。准确的证据边界见[证据说明](#证据说明)。
...
我想要的是一个足够小的检查点，能打断这些问题，又不会把 Agent 变成长期唱反调的角色。它优化的是决策质量，不是分歧数量。只有检查的预期决策价值高于延迟、复杂度和沟通成本时，才值得继续。任务长度不是判断依据。
...
在 `0.1.1` 中，我建议只在重要节点低频显式调用。隐式发现仍是尚未验证的实验能力；更窄的 Skill 描述用于排除日常、可逆且测试充分的工作。
...
我把 `0.1.1` 当作实验版本。是否能改善 Agent 行为，还需要隔离正确的对照测试。面对重要的生产决策，也不应该只依赖这一个 Skill。
